        ...
        try {
          ...
          if (hasFunctionCall && functionCallArgs) {
            try {
              ...
              // VALIDATION 1: Check if updates are actually provided
              ...
              const hasPlaceholderText = updateType === 'market_shift' && updates?.strategy?.persona && (
                ...
                updates.strategy.persona.name?.toLowerCase().includes('update the persona') ||
                updates.strategy.persona.location?.toLowerCase().includes('reflect') ||
                updates.strategy.persona.country?.toLowerCase().includes('reflect')
              );

              if (hasPlaceholderText) {
                console.error('❌ [Copilot] CRITICAL: AI returned placeholder text for persona!');
                console.error('❌ [Copilot] Placeholder persona:', JSON.stringify(updates.strategy?.persona, null, 2));

                // Send error message to user
                controller.enqueue(encoder.encode('\n\n[Error: I couldn\'t generate the complete persona details for the new market. Please try again with a more specific request, for example: "Change to Helsinki, Finland market for SaaS companies"]'));
              ...
              const updatedManifest = await updateBrandManifest(flowId, updates, updateType || 'refinement');
...
async function generateFallbackUpdates(
  ...
  if (updateType === 'styling') {
    ...
    const countryMatch = reasoning.match(/(?:india|mexico|brazil|china|japan|germany|france|spain|uk|united kingdom|usa|united states|finland|finnish|sweden|norway|denmark)/i);
    ...
    if (locationMatch || countryMatch) {
      ...
      console.log('🔄 [Copilot] Generating complete persona for market_shift:', { location, country });

      // Generate culturally appropriate persona data
      const personaExamples: Record<string, { name: string; role: string; company: string; industry: string }> = {
        'finland': { name: 'Mika Virtanen', role: 'Chief Design Officer', company: 'Nordic Creative Studio', industry: 'Design & Creative Services' },
        'finnish': { name: 'Mika Virtanen', role: 'Chief Design Officer', company: 'Nordic Creative Studio', industry: 'Design & Creative Services' },
        'helsinki': { name: 'Mika Virtanen', role: 'Chief Design Officer', company: 'Nordic Creative Studio', industry: 'Design & Creative Services' },
        'mexico': { name: 'Carlos Rodriguez', role: 'Director de Cumplimiento', company: 'Grupo Financiero México', industry: 'Financial Services' },
        'india': { name: 'Rajesh Kumar', role: 'Founder', company: 'CloudSync Inc', industry: 'SaaS' },
        'bangalore': { name: 'Rajesh Kumar', role: 'Founder', company: 'CloudSync Inc', industry: 'SaaS' },
        'germany': { name: 'Anna Schmidt', role: 'Product Manager', company: 'MarketLink AG', industry: 'Marketplace Platforms' },
        'berlin': { name: 'Anna Schmidt', role: 'Product Manager', company: 'MarketLink AG', industry: 'Marketplace Platforms' },
        'uk': { name: 'James Thompson', role: 'CEO', company: 'London Tech Solutions', industry: 'E-commerce' },
        'london': { name: 'James Thompson', role: 'CEO', company: 'London Tech Solutions', industry: 'E-commerce' },
        'usa': { name: 'Sarah Johnson', role: 'VP of Engineering', company: 'Silicon Valley Innovations', industry: 'Technology' },
        'united states': { name: 'Sarah Johnson', role: 'VP of Engineering', company: 'Silicon Valley Innovations', industry: 'Technology' },
      };

      const locationKey = location.toLowerCase();
      const countryKey = country.toLowerCase();
      const personaData = personaExamples[locationKey] || personaExamples[countryKey] || {
        name: 'Alex Johnson',
        role: 'CEO',
        company: `${location} Enterprises`,
        industry: manifest.strategy?.persona?.industry || 'Technology'
      };

      // Extract industry from reasoning if mentioned
      const industryMatch = reasoning.match(/(?:for|targeting)\s+([a-z\s]+(?:agencies|companies|platforms|services|startups))/i);
      if (industryMatch) {
        personaData.industry = industryMatch[1].trim();
      }

      // Generate market-appropriate pain points and goals
      const painPoints = [
        `Managing ${personaData.industry.toLowerCase()} operations in ${country}`,
        'Scaling business efficiently',
      ...
      // Full persona update with all required fields
      if (manifest.strategy?.persona) {
        ...
        console.log('✅ [Copilot] Generated complete persona for market_shift:', updates.strategy.persona);
      }
    } else {
